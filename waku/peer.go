...
func (p *Peer) handshake() error {
	...
	go func() {
		opts := statusOptions{
			...
			TopicInterest:        p.host.TopicInterest(),
			RateLimits:           &rateLimits,
			LightNodeEnabled:     &isLightNode,
			ConfirmationsEnabled: &confirmationsEnabled,
		}
		pow := p.host.MinPow()
		p.logger.Info("GOT POW", zap.Any("Pow", pow))
		if pow != 0 {
			opts.SetPoWRequirementFromF(pow)
		}
		errc <- p2p.SendItems(p.ws, statusCode, ProtocolVersion, opts.WithoutDefaults())
	}()
...
func (p *Peer) notifyAboutPowRequirementChange(pow float64) error {
	...
	p.logger.Info("NOTIFYING of changes", zap.Any("pow", i))
...
func (p *Peer) topicOrBloomMatch(env *Envelope) bool {
	return p.topicInterestMatch(env) || p.bloomMatch(env)
}
